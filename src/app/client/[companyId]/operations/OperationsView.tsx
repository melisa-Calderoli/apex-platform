...
import { Sparkles, Loader2, Target, CheckCircle2, Circle, Clock, AlertTriangle } from "lucide-react";
...
export default function OperationsView({
  ...
  const [tab, setTab] = useState<"timeline" | "client" | "internal" | "objectives">("timeline");
    ...
    <div>
      ...
      {tab === "timeline" && (
        <div className="space-y-6">
          ...
          {/* Timeline list */}
          <div className="bg-white border border-[#e5e5e0] rounded-xl p-6">
            <h3 className="font-medium text-black mb-4">Cronograma de acciones</h3>
            {actions.length === 0 ? (
              <p className="text-sm text-[#6b7280]">No hay acciones registradas todavia.</p>
            ) : (
...
function ActionRow({ action }: { action: Action }) {
  ...
  return (
    <div className="flex items-start gap-3 p-3 border border-[#e5e5e0] rounded-lg hover:bg-[#fafaf7] transition">
