  ...
  return (
    <div className="p-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div className="bg-[#1a1a35] border border-[#2a2a4d] rounded-xl p-6 text-center">
          <p className="text-xs uppercase tracking-wider text-[#8b8ba7] mb-4">Progreso del Plan</p>
          <div className="relative w-32 h-32 mx-auto">
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span className="text-3xl font-bold text-[#c9a84c] font-[family-name:var(--font-playfair)]">
                {actionsProgress}%
              </span>
            </div>
          </div>
          <p className="text-xs text-[#8b8ba7] mt-3">
            {actionsDone} de {actionsTotal} acciones completadas
          </p>
        </div>

        <div className="bg-[#1a1a35] border border-[#2a2a4d] rounded-xl p-6">
          <p className="text-xs uppercase tracking-wider text-[#8b8ba7] mb-2">Objetivos SMART</p>
          <p className="text-4xl font-bold text-[#f1f1f5] font-[family-name:var(--font-playfair)]">
            {objectives.length}
          </p>
          <div className="mt-4 space-y-2">
            <StatRow label="Completados" value={objectives.filter((o) => o.status === "completed").length} color="text-green-400" />
            <StatRow label="En progreso" value={objectives.filter((o) => o.status === "in_progress").length} color="text-[#c9a84c]" />
            <StatRow label="En riesgo" value={objectives.filter((o) => o.status === "at_risk").length} color="text-red-400" />
          </div>
        </div>

        <div className="bg-[#1a1a35] border border-[#2a2a4d] rounded-xl p-6">
          <p className="text-xs uppercase tracking-wider text-[#8b8ba7] mb-2">Estado</p>
      ...
      <div className="bg-[#1a1a35] border border-[#2a2a4d] rounded-xl p-6">
        ...
        {critical.length === 0 ? (
          <p className="text-[#8b8ba7] text-sm">No hay acciones criticas pendientes</p>
        ) : (
          <div className="space-y-2">
            {critical.map((a) => (
              <div key={a.id} className="flex items-center gap-3 p-3 bg-[#0a0a18] rounded-lg">
                <div className={`w-2 h-2 rounded-full ${
                  a.priority === "critical" ? "bg-red-400" : "bg-amber-400"
                }`} />
                <div className="flex-1">
                  <p className="text-sm text-[#f1f1f5]">{a.title}</p>
                  <p className="text-xs text-[#8b8ba7]">
                    {a.owner || "Sin asignar"} {a.due_date && `— Vence: ${a.due_date}`}
                  </p>
                </div>
                <span className="text-xs px-2 py-1 bg-[#2a2a4d] rounded text-[#8b8ba7]">{a.status}</span>
              </div>
            ))}
          </div>
        )}
...
function StatusItem({ label, done }: { label: string; done: boolean }) {
  return (
    <div className="flex items-center gap-2 text-sm">
      <div className={`w-2 h-2 rounded-full ${done ? "bg-green-400" : "bg-[#2a2a4d]"}`} />
      <span className={done ? "text-[#f1f1f5]" : "text-[#8b8ba7]"}>{label}</span>
    </div>
  );
}
...
function QuickCard({
  ...
  if (disabled) {
    return (
      <div className="bg-[#1a1a35] border border-[#2a2a4d] rounded-xl p-5 opacity-40">
        <div className="text-[#8b8ba7] mb-2">{icon}</div>
        <p className="text-[#f1f1f5] font-medium">{label}</p>
        <p className="text-xs text-[#8b8ba7] mt-1">Bloqueado</p>
      </div>
    );
  }
  return (
    <Link
      href={href}
      className="group bg-[#1a1a35] border border-[#2a2a4d] rounded-xl p-5 hover:border-[#c9a84c]/40 transition"
    >
      <div className="flex items-start justify-between mb-2">
        <div className="text-[#c9a84c]">{icon}</div>
        <ArrowRight size={16} className="text-[#8b8ba7] group-hover:text-[#c9a84c] group-hover:translate-x-1 transition" />
      </div>
      <p className="text-[#f1f1f5] font-medium">{label}</p>
      <p className="text-xs text-[#8b8ba7] mt-1">{description}</p>
    </Link>
  );
}
